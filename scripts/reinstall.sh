#!/usr/bin/env bash
# Rebuild discworld-mapper.mallardx and install it to Mallard's dev plugin dir.
...
PLUGIN_NAME="discworld-mapper"
PLUGIN_ID="net.mallard.${PLUGIN_NAME}"

# Platform app-data resolution. macOS only for now (matches existing Mallard
# scripts). Linux path lands when a user actually needs it.
case "$(uname -s)" in
...
# Bundle the four asset roots: plugin.toml + src + ui + (no scripts/, no .git).
